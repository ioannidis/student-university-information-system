...
/**
 * Handles student grade requests.
 */
...
public class StudentGradeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (courseId != null) {
            ...
            if (grade == null)
                grade = new Grade(course.getCourseId(), course.getTitle(), course.getEcts(),
                        course.getTeachingHours(), course.getInstructorUsername(), -1,
                        course.getSemester(), course.getDepartmentId());
        ...
        switch (sortBy) {
            case "list": {
                List<Grade> grades =
                        gradeService.getGradesByList(user.getUsername(), user.getDepartmentId());

                request.setAttribute("grades", grades);
                request.setAttribute("sortBy", "list");
                request.getRequestDispatcher("WEB-INF/views/grade/index.jsp").forward(request,
                        response);
                break;
            }
            case "average": {
                ...
                List<Grade> grades =
                        gradeService.getGradesByAverage(user.getUsername(), user.getDepartmentId());
            ...
            case "semester":
            default: {
                List<Grade> grades = gradeService.getGradesBySemester(user.getUsername(),
